...
void setupAddressStruct(struct sockaddr_in* address, int portNumber, char* hostname){
	...
	//printf("port in client: %i\n", ntohs(address->sin_port));
  	...
  	if (hostInfo == NULL) { 
    		error("DEC_CLIENT: ERROR, no such host\n", 2); 
	}
...
int main(int argc, char *argv[]) {
	...
	if(plainFile == -1){
		fprintf(stderr, "DEC_CLIENT: Failed to open %s\n", argv[1]);
		exit(1);
	}	 
	...
	if(strlen(keyBuffer) < strlen(textBuffer)){
		error("DEC_CLIENT:key size too short for the plaintext size", 1);
	}	
	...
	if(badtext == true || badkey == true){
		error("DEC_CLIENT: Bad character found", 1);
	}
	...
	// put all data together and send it at once later
  	...
  	if (connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0){
    		error("DEC_CLIENT: ERROR connecting", 2);
  	}
 	...
 	// Send message to server
  	// Write to the server
  	char messg[] =  "Connecting from decrypt client\n";
  	int charsWrite = 0;
	charsWrite =  send(socketFD, messg, strlen(messg),0);
	if(charsWrite < 0){
		error("DEC_CLIENT: ERROR writing greeting message to socket", 0);
	}
	...
	textWritten = send(socketFD, largeBuffer, strlen(largeBuffer), 0);
        if (textWritten < 0){
               	error("DEC_CLIENT: ERROR writing data to socket", 1);
        }
